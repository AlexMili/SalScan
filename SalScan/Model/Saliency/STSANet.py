...
class STSANetwork(AbstractModel):
    """
    Spatio-Temporal Self-Attention Network (STSANet) for video saliency prediction.
    This model generates a saliency map using a 32 videos frames context-window.
    This model is Many-to-One.

    Parameters:
        options (dict): Configuration options to set `path_to_weights`.

    Attributes:
        name (str): The name of the model, 'STSA_Network'.
    ...
    type = "video_saliency"
    wrapper = "python"
    params_to_store = ["image_height", "image_width", "path_to_weights"]

    def __init__(self, options):
        super().__init__()

        self._config = {**options}
        path_to_weigths = self._config.get("path_to_weights", "")
        if os.path.exists(path_to_weigths) is False:
            raise FileNotFoundError(
                "\n ❌ 'path_to_weigths' either an empty or non valid directory."
                " Weights can be found from https://drive.google.com/drive/folders/1Wn4vilAKEplNnkGb06UsPOuDI0JdVtrb"
            )
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model = STSANet().to(self.device)

        state_dict = torch.load(path_to_weigths, map_location=self.device)
        # weights where stored from nn.DataParallel without using
        # torch.save(model.module.state_dict(), PATH). Therefore state_dict needs to be
        # recreated to remove "module." from each weight.
        new_state_dict = OrderedDict()
        for k, v in state_dict.items():
            new_state_dict[k.replace("module.", "")] = v
        self.model.load_state_dict(new_state_dict)

    def run(self, imgs: np.array, params) -> np.ndarray:
        """
        Generates the saliency map.

        Args:
            img (np.array): The input array which is a stack of 32 frames.
            params (dict): model's run parameters
        """
        imgs = torch.from_numpy(imgs)[None, ...].permute(0, 2, 1, 3, 4)
        imgs = imgs.to(self.device)
        salmap = self.model(imgs).cpu().detach().numpy()
        salmap = normalize(salmap, method="range")
        salmap = np.squeeze(salmap)

        return salmap


# https://github.com/WZq975/STSANet
class STSANet(nn.Module):
    def __init__(self):
        ...
        self.branch0_pre = nn.Sequential(
            nn.Conv3d(
                192, 192, kernel_size=(4, 1, 1), stride=(4, 1, 1), padding=(0, 0, 0)
            ),
        ...
        self.branch0_mid = nn.Sequential(
            nn.Conv3d(
                192, 192, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch1_pre = nn.Sequential(
            nn.Conv3d(
                480, 480, kernel_size=(4, 1, 1), stride=(4, 1, 1), padding=(0, 0, 0)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch1_mid = nn.Sequential(
            nn.Conv3d(
                480, 240, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch2_pre = nn.Sequential(
            nn.Conv3d(
                832, 832, kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=(0, 0, 0)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch2_mid = nn.Sequential(
            nn.Conv3d(
                832, 416, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch3_mid = nn.Sequential(
            nn.Conv3d(
                1024, 512, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
        )
        ...
        self.branch3_beh_0 = nn.Sequential(
            nn.Conv3d(
                512, 416, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
            self.upsample,
        )
        ...
        self.SA_0 = nn.Sequential(
            nn.Conv3d(
                416 + 416, 1, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.Sigmoid(),
        )
        self.SA_1 = nn.Sequential(
            nn.Conv3d(
                240 + 240, 1, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.Sigmoid(),
        )
        ...
        self.CA_0 = nn.Sequential(
            self.avg_pooling_4,
            nn.Conv3d(
                416 + 416, 208, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.LayerNorm([208, 4, 1, 1]),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                208, 416 + 416, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.Sigmoid(),
        )
        self.CA_1 = nn.Sequential(
            self.avg_pooling_4,
            nn.Conv3d(
                240 + 240, 120, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.LayerNorm([120, 4, 1, 1]),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                120, 240 + 240, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.Sigmoid(),
        )
        self.CA_2 = nn.Sequential(
            self.avg_pooling_4,
            nn.Conv3d(
                192 + 192, 96, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.LayerNorm([96, 4, 1, 1]),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                96, 384, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
            nn.Sigmoid(),
        )
        ...
        self.inc_0_0 = nn.Sequential(
            nn.Conv3d(
                416, 240, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
        )
        self.inc_0_1 = nn.Sequential(
            nn.Conv3d(
                416, 240, kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)
            ),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                240, 240, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
        )
        self.inc_0_2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(
                240, 240, kernel_size=(1, 5, 5), stride=(1, 1, 1), padding=(0, 2, 2)
            ),
        )
        ...
        self.inc_1_0 = nn.Sequential(
            nn.Conv3d(
                240, 192, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
        )
        self.inc_1_1 = nn.Sequential(
            nn.Conv3d(
                240, 192, kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)
            ),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                192, 192, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
        ...
        self.inc_1_2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(
                192, 192, kernel_size=(1, 5, 5), stride=(1, 1, 1), padding=(0, 2, 2)
            ),
        )
        ...
        self.inc_2_0 = nn.Sequential(
            nn.Conv3d(
                192, 192, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)
            ),
        )
        self.inc_2_1 = nn.Sequential(
            nn.Conv3d(
                192, 192, kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)
            ),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                192, 192, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
        ...
        self.readout_1 = nn.Sequential(
            nn.Conv3d(
                192, 64, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)
            ),
            nn.ReLU(inplace=True),
            self.upsample,
        )
    ...
    def stsa(
        self, input_a, input_b, emb_a_q, emb_a_k, emb_a_v, emb_b_q, emb_b_k, emb_b_v
    ):
    ...
    def forward(self, x):
        ...
        base1_0_down, base1_1_down, base1_2_down, base1_3_down = base1_down.split(
            1, dim=2
        )
        out1_0, out1_1 = self.stsa(
            base1_0_down,
            base1_1_down,
            self.branch0_0_a_q,
            self.branch0_0_a_k,
        ...
        add = self.unpool_2(
            torch.cat((out1_0, out1_1, out1_2, out1_3), dim=2), indices=indices
        )
